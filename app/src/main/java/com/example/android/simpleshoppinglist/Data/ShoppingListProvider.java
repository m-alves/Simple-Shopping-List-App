...
public class ShoppingListProvider extends ContentProvider{
    ...
    private static final int WAREHOUSE = 100;

    /** URI matcher code for the content URI for a single pet in the pets table */
    private static final int WAREHOUSE_ID = 101;
    ...
    static {
        ...
        // of the pets table.
        sUriMatcher.addURI(ShoppingListContract.CONTENT_AUTHORITY, ShoppingListContract.PATH_WAREHOUSE, WAREHOUSE);
        ...
        sUriMatcher.addURI(ShoppingListContract.CONTENT_AUTHORITY, ShoppingListContract.PATH_WAREHOUSE + "/#", WAREHOUSE_ID);
    }
    ...
    public boolean onCreate() {
        mDbHelper = new ShoppingListDbHelper(getContext());
        return true;
    }
    ...
    public Uri insert(Uri uri, ContentValues contentValues) {
        ...
        switch (match) {
            case WAREHOUSE:
                return insertItemWarehouse(uri, contentValues);
            default:
     ...
     */
    private Uri insertItemWarehouse(Uri uri, ContentValues values) {
        // Check that the name is not null
        String name = values.getAsString(WarehouseEntry.COLUMN_WAREHOUSE_NAME);
        if (name == null) {
            throw new IllegalArgumentException("Pet requires a name");
        }

        // If the weight is provided, check that it's greater than or equal to 0 kg
        String date = values.getAsString(WarehouseEntry.COLUMN_WAREHOUSE_DATE);
        if (date == null) {
            throw new IllegalArgumentException("Pet requires valid weight");
        }

        // If the weight is provided, check that it's greater than or equal to 0 kg
        int inlist = values.getAsInteger(WarehouseEntry.COLUMN_WAREHOUSE_INLIST);
    ...
    private int updateWarehouse(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(WarehouseEntry.COLUMN_WAREHOUSE_NAME)) {
            String name = values.getAsString(WarehouseEntry.COLUMN_WAREHOUSE_NAME);
            if (name == null) {
                throw new IllegalArgumentException("Pet requires a name");
            }
        ...
        if (values.containsKey(WarehouseEntry.COLUMN_WAREHOUSE_DATE)) {
        ...
        // If the {@link PetEntry#COLUMN_PET_WEIGHT} key is present,
        // check that the weight value is valid.
        if (values.containsKey(WarehouseEntry.COLUMN_WAREHOUSE_INLIST)) {
            // Check that the weight is greater than or equal to 0 kg
            Integer date = values.getAsInteger(WarehouseEntry.COLUMN_WAREHOUSE_INLIST);
            if (date == null) {
                throw new IllegalArgumentException("Pet requires valid weight");
            }
        }
